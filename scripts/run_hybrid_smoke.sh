...
mpirun --bind-to none -np 2 ./hybrid_mpi_cuda "$dataset" \
    ...
    --comm reduce-bcast --batch-size "$batch_size" --epochs "$epochs" --smoke-test "${validate[@]}" \
...
mpirun --bind-to none -np 3 ./hybrid_mpi_cuda "$dataset" \
    ...
    --comm reduce-bcast --overlap on --batch-size "$batch_size" --epochs "$epochs" --smoke-test "${validate[@]}" \
